...
Calcula tránsitos minuto a minuto para mayor precisión.
"""
import json
import time
...
class AllTransitsCalculator:
    def __init__(self, natal_data: dict, timezone_str: str = "UTC"):
        """
        Inicializa el calculador de tránsitos estándar.
    ...
    def calculate_all(self, start_date: datetime = None, end_date: datetime = None) -> list:
        """
        Calcula todos los tránsitos para el período especificado.
        ...
        while current <= end_date:
            # Calcular y mostrar progreso cada 1%
            progress = int((current - start_date).days * 100 / total_days)
            if progress > last_reported_progress:
                elapsed = time.time() - start_time
                eta = (elapsed / (progress + 0.1)) * (100 - progress) if progress > 0 else 0
                print(f"Progreso: {progress}% ({current.strftime('%Y-%m-%d')}) - "

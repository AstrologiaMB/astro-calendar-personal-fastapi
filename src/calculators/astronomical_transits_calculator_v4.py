...
# Símbolos de planetas para tránsitos por casas
PLANET_SYMBOLS = {
    chart.JUPITER: "♃",
    chart.SATURN: "♄", 
    chart.URANUS: "♅",
...
class AstronomicalTransitsCalculatorV4:
    """
    Versión 4.0 (Experimental) del calculador de tránsitos astronómico.
    ...
    def __init__(self, natal_data: dict):
        """
        Inicializa el calculador de tránsitos astronómico v4.0 (Experimental con Caching).
        ...
        # Agregar cúspides de casas natales
        self.house_cusps = {}
        if 'houses' in natal_data:
            for house_num, house_data in natal_data['houses'].items():
                self.house_cusps[int(house_num)] = house_data['longitude']
            print(f"Cargadas {len(self.house_cusps)} cúspides de casas natales")
        else:
            print("No se encontraron datos de casas natales")
        
        # Inicializar caché para efemérides
        self.ephemeris_cache = {}
        
        # Instanciar el calculador especializado de Luna Progresada
        self.progressed_moon_calculator = ProgressedMoonTransitsCalculator(natal_data)
        
    def _get_cached_ephemeris(self, planet_id, jd):
        """
        Obtiene los datos de efemérides para un planeta y fecha juliana,
        utilizando un caché para evitar cálculos repetidos.
        """
        cache_key = (planet_id, jd)
        if cache_key in self.ephemeris_cache:
            return self.ephemeris_cache[cache_key]
        else:
            # Calcular, almacenar en caché y devolver
            planet_data = ephemeris.planet(planet_id, jd)
            self.ephemeris_cache[cache_key] = planet_data
            return planet_data
    ...
    def get_planet_speed_category(self, planet_id, date):
        ...
        planet_data = self._get_cached_ephemeris(planet_id, jd) # Use cache
    ...
    def find_exact_aspect_time(self, planet_id, natal_position, aspect_angle, start_jd, end_jd, max_iterations=20):
        ...
        for iteration in range(max_iterations):
            ...
            data1 = self._get_cached_ephemeris(planet_id, start_jd) # Use cache
            data2 = self._get_cached_ephemeris(planet_id, end_jd) # Use cache
            ...
            if diff1 * diff2 <= 0:
                ...
                mid_data = self._get_cached_ephemeris(planet_id, mid_jd) # Use cache
    ...
    def get_adaptive_sampling_interval(self, planet_id, aspect_type, date, last_speed=None):
        ...
        final_hours = base_hours * aspect_factor * speed_factor * acceleration_factor
        # Asegurar un intervalo mínimo (e.g., 15 minutos)
        min_hours = 0.25
        final_hours = max(min_hours, final_hours)
    ...
    def refine_direction_change(self, planet_id, start_date, end_date, max_iterations=10):
        ...
        start_data = self._get_cached_ephemeris(planet_id, start_jd) # Use cache
        end_data = self._get_cached_ephemeris(planet_id, end_jd) # Use cache
        ...
        if orb <= max_orb:
            ...
            if orb > settings.exact_orb:
                ...
                if exact_jd:
                    ...
                    exact_data = self._get_cached_ephemeris(planet_id, exact_jd) # Use cache
            ...
            if abs(speed) <= 0.0001:
                ...
                prev_data = self._get_cached_ephemeris(planet_id, prev_jd) # Use cache
    ...
    def estimate_transit_dates(self, planet_id, natal_position, aspect_angle, year, start_date=None, end_date=None):
        ...
        planet_data = self._get_cached_ephemeris(planet_id, jd_start) # Use cache
            ...
            for _ in range(cycles):
                if current_date <= end_date:
                    ...
                    if days_since_last_recalc >= recalc_interval:
                        ...
                        current_data = self._get_cached_ephemeris(planet_id, jd_current) # Use cache
    ...
    def get_current_house_for_planet(self, planet_id, date):
        """
        Determina en qué casa natal está un planeta en una fecha específica.
        ...
        planet_longitude = planet_data['lon']
        
        return self.get_current_house_for_planet_position(planet_longitude)
    
    def get_current_house_for_planet_position(self, longitude):
        """
        Determina en qué casa natal está una posición específica.
        
        Args:
            longitude: Longitud en grados (0-360)
        ...
        # Encontrar la casa basada en las cúspides
        for house in range(1, 13):
            next_house = house + 1 if house < 12 else 1
            cusp_current = self.house_cusps[house]
            cusp_next = self.house_cusps[next_house]
            
            # Manejar el cruce del 0° (Aries)
            if cusp_current > cusp_next:  # Cruza 0°
    ...
    def calculate_house_transits_state(self, reference_date):
        """
        Calcula el estado actual de tránsitos de largo plazo:
        - Planetas lentos por casas natales
        - Luna progresada (signo + casa natal)
        
        Args:
            reference_date: Fecha de referencia para el cálculo
            
        Returns:
            AstroEvent con el estado de tránsitos de largo plazo
        """
        if not self.house_cusps:
            print("No hay datos de casas natales disponibles")
            return None
        ...
        for planet_id in planets:
            try:
                ...
                planet_symbol = PLANET_SYMBOLS.get(planet_id, "")
                house_meaning = HOUSE_MEANINGS.get(current_house, f"Casa {current_house}")
        ...
        try:
            # Usar el calculador especializado en lugar de la implementación duplicada
            progressed_moon_pos = self.progressed_moon_calculator._calculate_progressed_moon_position(reference_date)
            
            # Signo y grado
            moon_sign = AstronomicalConstants.get_sign_name(progressed_moon_pos)
            moon_degree = round(progressed_moon_pos % 30, 1)
            
            # Casa natal donde está transitando
            moon_house = self.get_current_house_for_planet_position(progressed_moon_pos)
            house_meaning = HOUSE_MEANINGS.get(moon_house, f"Casa {moon_house}")
            ...
            print(f"Luna Progresada en {moon_sign} {moon_degree}° - Casa {moon_house} ({house_meaning}) [USANDO CALCULADOR ESPECIALIZADO]")
            
        except Exception as e:
            print(f"Error calculando luna progresada con calculador especializado: {e}")
        
        if not house_states:
            return None
        
        # Crear evento único con toda la información
        event = AstroEvent(
            fecha_utc=reference_date,
            tipo_evento=EventType.TRANSITO_CASA_ESTADO,
            descripcion="Estado actual de tránsitos de largo plazo",
            metadata={'house_transits': house_states},
            timezone_str=self.user_timezone.key
        )
    ...
    def calculate_all(self, start_date: datetime = None, end_date: datetime = None, planets_to_check=None) -> list:
        ...
        print("\n🚀 DEBUG: Usando AstronomicalTransitsCalculatorV4")
        print("\nCalculando tránsitos con método astronómico v4.0...")
        ...
        all_astro_events = self.convert_to_astro_events(filtered_transits)

        # Filtrar eventos de aspecto para incluir solo "Exacto" o "Estacionario"
        # (Esta lógica estaba antes en main.py, se mueve aquí para encapsulamiento)
        final_events_to_return = []
        for event in all_astro_events:
            if event.tipo_evento == EventType.ASPECTO:
                # Verificar si el estado es "Exacto" o si el movimiento es "Estacionario"
                is_exact = 'estado' in event.metadata and event.metadata['estado'] == ASPECT_STATE[calc.EXACT]
                is_stationary = 'movimiento' in event.metadata and event.metadata['movimiento'] == MOVEMENT_NAMES[calc.STATIONARY]
                if is_exact or is_stationary:
        ...
        print(f"\nCálculo astronómico v4.0 completado en {elapsed:.2f} segundos")
        print(f"Total de eventos calculados (antes de filtro exacto/estacionario): {len(all_astro_events)}")
        print(f"Total de eventos finales (incluyendo casas): {len(final_events_to_return)}")

        return final_events_to_return

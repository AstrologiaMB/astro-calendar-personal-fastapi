"""
Módulo para generar el análisis de eclipses solares en casas natales.
"""
import json
import csv

def generate_solar_eclipse_houses_report(natal_file: str, events_file: str, output_file: str = 'natal_eclipsesolar.csv'):
    """
    Genera un reporte de los eclipses solares y las casas natales donde ocurren.
    ...
    # Cargar datos de eclipses solares
    solar_eclipses = []
    with open(events_file, 'r') as f:
        reader = csv.DictReader(f)
        for row in reader:
            if row['tipo_evento'] == 'Eclipse Solar':
                ...
                solar_eclipses.append({
                    ...
                    'signo': row['signo'],
                    'grado': grado,
                    'tipo': 'total' if 'Total' in row['descripcion'] else 'parcial'
                })
    ...
    with open(output_file, 'w', newline='') as f:
        ...
        for eclipse in solar_eclipses:
            ...
            casa = determinar_casa_natal(eclipse['signo'], eclipse['grado'], natal_data['houses'])
            
            # Generar descripción
            desc = (f"El eclipse solar {eclipse['tipo'].lower()} del día {eclipse['fecha']} y hora {eclipse['hora']} "
...
def determinar_casa_natal(signo: str, grado: float, casas: dict) -> int:
    ...
    # Convertir posición del sol a grados absolutos (ya viene en español)
    pos_sol = _convertir_a_grados_absolutos(signo, grado)
    print(f"Posición absoluta del sol: {pos_sol}°")
    ...
    for i in range(len(casas_ordenadas)):
        ...
        if fin < inicio:
            ...
            if pos_sol >= inicio or pos_sol < fin:
                return casa_actual[0]
        # Caso normal
        elif inicio <= pos_sol < fin:
            return casa_actual[0]
